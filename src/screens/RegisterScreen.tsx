...
export const RegisterScreen = ({ onRegisterSuccess }: { onRegisterSuccess: () => void }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!validateName(nombre) || !validateName(apellido)) {
      setErrorMsg('Nombre y apellido deben contener solo letras y mínimo 2 caracteres.');
      setSuccessMsg('');
      return;
    }
    ...
    try {
      await axios.post('https://exchangebooks.up.railway.app/api/register', {
        name: nombre,
        lastname: apellido,
        email: correo,
        password: contrasena
